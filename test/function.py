    ...
    def schedule_attempt(self, op_graph, op_nodes, tiling_choices):
        ...
        while finished_count < len(self.nodes):
            ...
            while running_nodes and running_nodes[0][0] <= current_time:
                ...
                # Simplified memory release: assumes memory is freed upon task completion
...
def main():
    ...
    if get_inference_line:
        ...
        if initial_args:
            ...
            tiling_choices = {}
            for op_id, op_type, shape in op_nodes_info:
                if (op_type, shape) in scheduler.available_tilings and scheduler.available_tilings[(op_type, shape)]:
                    tiling_choices[op_id] = scheduler.available_tilings[(op_type, shape)][0]
                else: # Fallback if no tiling is defined for a requested op
                    print("[]")
                    return
                ...
                if is_success:
                    ...
                    try:
                        ...
                        if current_idx + 1 < len(available):
                            ...
                            op_type_change, shape_change = op_map[op_to_change]
                            if (op_type_change, shape_change) in scheduler.available_tilings:
