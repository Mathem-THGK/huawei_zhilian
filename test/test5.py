...
# 版本：最终稳健混合策略版
# 核心修正：
# 1. 修正了 Scheduler 类，采用正确的延迟内存释放模型，从根本上减少错误的死锁判断。
# 2. 实现了最稳健的启动流程：优先贪心，贪心失败则随机搜索，确保总能以一个可行解启动模拟退火。
# -----------------------------------------------------------------------------
...
class Scheduler:
    # -------------------------------------------------------------------------
    # [关键修正 1] 使用包含正确内存管理逻辑的 Scheduler 类
    # -------------------------------------------------------------------------
    def __init__(self):
        self.soc_info = {}
        self.op_lib = {}
    ...
    if get_inference_line:
        ...
        if initial_args:
            ...
            for op_id, (op_type, shape) in op_map.items():
                ...
                initial_choice[op_id] = best_tiling_id if best_tiling_id != -1 else available[0]

            initial_energy, initial_schedule = scheduler.schedule_attempt(op_graph, op_nodes_info, initial_choice)
            ...
            if initial_energy == float('inf'):
                ...
                while initial_energy == float('inf') and attempts < max_attempts:
                    initial_choice = {op_id: random.choice(scheduler.available_tilings[op_map[op_id]]) for op_id in op_map if op_map[op_id] in scheduler.available_tilings and scheduler.available_tilings[op_map[op_id]]}
                    initial_energy, initial_schedule = scheduler.schedule_attempt(op_graph, op_nodes_info, initial_choice)
                    attempts += 1
                if initial_energy == float('inf'):
            ...
            current_solution, current_energy, current_schedule = initial_choice, initial_energy, initial_schedule
            ...
            while temp > final_temp and (time.time() - start_of_main) < time_limit:
                ...
                if current_energy < best_energy:
                    ...
                    # 注意：best_schedule 应该和 best_energy/best_solution 同步更新
